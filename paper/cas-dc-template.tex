...
% ============================================================
\begin{highlights}
\item Recursive degradation exhibits a sharp pressure-dependent transition
\item Pressure thresholds differ by an order of magnitude across backbones
...
\item Reducing synthetic exposure by 5\% restores near-homeostatic behavior on Qwen
\end{highlights}
...
overview of the experimental pipeline. The distinguishing feature of this design
is that pressure manipulation (Stage~3) systematically varies training intensity
across multiple axes rather than evaluating a single fixed configuration. Each
axis produces an independent dose-response curve, and the resulting regime
classification (Stage~5) constitutes the primary empirical contribution.
...
($r=256$) is replicated with three seeds to confirm reproducibility; remaining
configurations were evaluated as part of the exploratory dose-response
sweep.}\label{tab:rank_sweep}
...
reproducible. Per-seed Gen10 values are 76.9\%, 78.2\%, and 82.1\%. To verify
that the observed homeostatic regime was not specific to the original run, we
independently repeated the $r=16$ configuration with three random seeds as part
of the QLoRA-versus-FFT experiments (Section~\ref{sec:res_fft}); all three runs
converged to 97.4\% Gen10 retention. The ranges do not overlap: the lowest
replicated $r=16$ value (97.4\%) exceeds the highest $r=256$ value (82.1\%) by
15.3 percentage points, demonstrating clear empirical separation under the
present experimental protocol.
...
Qwen~2.5~1.5B (seed~15, $K_0 = 78$). C$\to$W: facts lost; W$\to$C: facts
recovered.}\label{tab:transitions}
\footnotesize
...
Figure~\ref{fig:combined_traj} presents the retention trajectories for both
backbones side by side, enabling direct visual comparison of the regime
structure.

\vspace{6pt}
\begin{center}
  \includegraphics[width=0.95\columnwidth]{figs/fig_combined_trajectories.png}
  \captionof{figure}{K0 retention trajectories on (a)~Qwen~2.5~1.5B over ten
  generations and (b)~Gemma~3~1B over five generations. Thin lines show
  individual seeds; bold lines show means. Both backbones exhibit the same
  qualitative regime structure (homeostatic below threshold, degradative above),
  but the transition occurs at substantially different effective ranks
  (${\sim}$50 on Qwen versus ${\sim}$3--6 on
  Gemma~3).}\label{fig:combined_traj}
\end{center}
\vspace{6pt}
...
(i)~they interact jointly (higher rank at lower LR preserves stability),
...
\midrule
Rank governs transition & Rank sweep, 6 values, $N{=}3$ at endpoint \\
\addlinespace
...
established the joint interaction between update capacity and
