...
regained. We do not claim precise threshold locations or a universal
effective-rank value at which degradation begins; instead, we identify the
interval between adjacent tested settings where the observed regime transition
occurs. Single-seed observations are used as exploratory or supportive evidence
and are not used as standalone support for headline conclusions.
...
Several features of the dose-response curve (Fig.~\ref{fig:dose_response}) merit
attention. First, the
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figs/fig2_dose_response.pdf}
  \caption{Dose-response relationship between nominal adapter rank and recursive
  stability on Qwen~2.5~1.5B. Left panel: Gen10 K0 retention (\%) versus nominal
  rank. Right panel: mean effective rank versus nominal rank. Retention declines
  monotonically with increasing effective update capacity; the sharpest drop
  occurs between $r=128$ (bounded, 88.6\%) and $r=256$ (degradative,
  78.0\%).}\label{fig:dose_response}
\end{figure}
...
% TODO: Write discussion opening paragraph
