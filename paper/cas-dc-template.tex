...
and five independent seeds, we investigate how recursive factual retention
changes as adapter capacity, perturbation magnitude, and synthetic exposure
vary. Together, these experiments reveal a common empirical organizing
...
differing by approximately an order of magnitude across architectures
...
retention remains above 88\%; and (5) restoration of near-homeostatic behavior
through approximately 5\% reduction in synthetic exposure at the Qwen boundary
configuration. These findings suggest that recursive degradation is a
pressure-gated phenomenon whose stability depends on a multidimensional control
landscape, providing an empirical basis for monitoring and configuring recursive
training pipelines.
\end{abstract}
...
\item Pressure thresholds differ by an order of magnitude across backbones and are not universal
...
\begin{enumerate}
    ...
    confirmed across five independent seeds.
...
\begin{figure*}[t]
  ...
  \vspace{4pt}
  \caption{Overview of the experimental pipeline. Our study consists of five
  main stages: (1)~data preparation, including TriviaQA dataset splitting,
  $K_0$ factual baseline establishment, and evaluation protocol definition;
  (2)~recursive training over ten generations using QLoRA or FFT with a strict
  replace-only protocol across three backbones; (3)~systematic pressure
  manipulation along four axes: adapter rank ($r \in \{4,16,32,64,128,256\}$),
  learning rate ($10^{-6}$ to $2{\times}10^{-5}$), their joint interaction, and
  synthetic exposure reduction; (4)~comprehensive evaluation of factual
  retention, distributional shift, behavioral metrics, and effective rank at
  each generation; and (5)~regime classification into homeostatic, bounded, or
  degradative categories based on observed retention trajectories and
  distributional drift.}\label{fig:methodology}
\end{figure*}
...
\midrule
$r=4$   & 545K  & 5  & 96.2\%           & 3.34  & Homeo. \\
$r=16$  & 2.2M  & 10 & 94.9\% (N=3)     & 11.08 & Homeo. \\
$r=32$  & 4.4M  & 5  & 94.9\%           & 17.85 & Homeo. \\
$r=64$  & 8.7M  & 10 & 91.1\%           & 29.52 & Homeo. \\
$r=128$ & 17.4M & 10 & 88.6\%           & 50.16 & Bounded \\
$r=256$ & 34.9M & 10 & 78.0\% (N=3)     & 87.57 & Degrad. \\
\bottomrule
\end{tabular*}
\end{table}
...
effective rank 3.07), Gemma~3 remains homeostatic across five seeds (mean Gen5
...
between effective ranks 3 and 6, approximately an order of magnitude below the
...
The interaction is evident (Figure~\ref{fig:heatmap}): configurations that remain homeostatic under one
...
\begin{figure*}[t]
  ...
  stability. As effective training pressure increases (controlled by adapter
  rank, learning rate, and synthetic exposure), factual retention ($K_0$)
  transitions through three regimes: homeostatic (retention above 90\%,
  distribution stable), bounded (retention stable but output quality degrades),
  and degradative (progressive factual loss). The green, yellow, and red markers
  indicate representative operating points observed in the experiments. Dashed
  vertical lines mark the backbone-dependent regime boundaries. The transition
  is sharp rather than gradual, and its location varies by approximately an
  order of magnitude across the evaluated architectures.}\label{fig:etp}
\end{figure*}
...
single scalar predictor. Instead, we observe that each variable independently
...
We emphasize that the contribution is the existence and reproducibility of this
sharp transition, not the numerical threshold itself. We do not claim that the
...
We note that the evidence for treating these axes as manifestations of a
...
We therefore interpret effective training pressure not as a physical quantity
...
We emphasize that this diagnostic value does not rest on a causal claim. The
...
substantially (partial $\rho = -0.22$). We interpret output drift as a
...
low in homeostatic ones, regardless of whether it contributes to, or simply
...
pipelines. We organize these around three operational concerns: configuration
...
homeostatic through ten recursive generations with no sign of drift. Although
...
from its homeostatic operating regime, even if factual retention metrics appear
...
We note that this scale is consistent with prior model-collapse studies
...
Gen10 are unknown: configurations classified as homeostatic over this horizon
...
backbones, or with different datasets has not been established. We note that the
restriction to a single boundary configuration is deliberate: the purpose of
...
restoring homeostatic levels, indicating that the degradation measured by $K_0$
...
configurations. We therefore interpret the contribution as evidence for a
...
is monitored and maintained below the relevant backbone-dependent threshold. We
note that the present contribution is the identification of this reproducible
