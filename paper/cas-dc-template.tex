...
training rather than a language-specific artifact.
...
recursive generations for any nonzero fraction of synthetic data.
...
\subsection{Knowledge degradation under recursive training}
...
grounding. Beyond factual degradation itself, \citet{leclerc2025recursive}
...
same verifier. At the data-composition stage, \citet{gerstgrasser2024} showed
...
available. At the training-objective stage, \citet{zibakhsh2024} introduced
...
sequential task updates \citep{kirkpatrick2017,delange2022}, recent work
...
and forgets less'' than full fine-tuning. Emerging frameworks attempt to
operationalize this stability through structural matrix constraints
\citep{fawi2024curlora} or localized memory slots \citep{le2026procl}. In
...
layers function as key-value memories, suggesting that low-rank constraints on
specific modules may bottleneck the network's ability to overwrite foundational
facts.
...
\begin{table}[width=.95\linewidth,cols=3,pos=htbp]
\caption{Experimental axes and their corresponding pressure
components.}\label{tab:axes}
\begin{tabular*}{\tblwidth}{@{} lp{1.8cm}p{3.4cm}@{} }
\toprule
Axis & Variable & Range \\
\midrule
1. Capacity & QLoRA rank & $r \in \{4,16,32,64,128,256\}$ \\
2. Magnitude & FFT learning rate & $10^{-6}$ to $2{\times}10^{-5}$ \\
3. Method & QLoRA vs FFT & Matched pert., $N{=}3$, Gen10 \\
...
metrics. Distinct-$n$~\citep{li2016} computes the ratio of unique $n$-grams to
...
effect sizes, monotonic dose-response behavior, and per-seed consistency. The
...
\vspace{6pt}
\begin{center}
  \includegraphics[width=\linewidth]{figs/fig1_trajectories.png}\\[2pt]
  \includegraphics[width=\linewidth]{figs/fig2_dose_response.png}
...
\vspace{6pt}
\begin{center}
  \includegraphics[width=\columnwidth]{figs/fig3_cross_backbone.png}
...
\subsubsection{QLoRA versus FFT at approximately matched perturbation}
...
\vspace{6pt}
\begin{center}
  \includegraphics[width=\columnwidth]{figs/fig4_fft_vs_qlora.png}
...
\subsubsection{Bounded regime ($r=128$): a retention/distribution dissociation}
...
\vspace{6pt}
\begin{center}
  \includegraphics[width=\columnwidth]{figs/fig5_distributional.png}
...
\vspace{6pt}
\begin{center}
  \includegraphics[width=\columnwidth]{figs/fig6_interventions.png}
...
\subsection{Effective training pressure as an organizing framework}\label{sec:disc_pressure}
...
forgets less'' relative to full fine-tuning. Our dose-response results refine
...
generations prevents collapse, operating on the data-composition axis. Our
...
multiple independent control axes.
...
\subsection{Practical implications for recursive training systems}\label{sec:disc_practical}
...
question-answer pairs. The findings may not extend to long-form generation,
open-ended dialogue, code synthesis, or other domains where output structure and
evaluation criteria differ substantially from factoid recall.
...
architectural ratios (hidden dimension, depth, head count), and none aligned the
thresholds between Qwen and Gemma~3. The raw effective-rank threshold is
...
introducing residual confounding in the method comparison.
...
backbones, or with different datasets has not been established.
...
and deployment conditions, has not been assessed.

Finally, effective training pressure is proposed as an empirical organizing
framework rather than a predictive mathematical model. Whether a quantitative
formulation combining the three pressure components into a single scalar
predictor exists remains an open question.
...
The experimental results consistently show that recursive stability depends on
three controllable dimensions of the training process: update capacity,
perturbation magnitude, and synthetic exposure. Across the evaluated backbones,
increasing pressure along any of these dimensions moves the system from
homeostatic to degradative behavior, while modest reductions near the regime
boundary restore bounded or near-homeostatic performance. The regime transition
is sharp, backbone-dependent, and reproducible across seeds. Complementary
distributional diagnostics further reveal that output quality can degrade
substantially before severe factual loss becomes apparent, providing additional
signals for monitoring recursive training beyond accuracy alone.
...
Rather, it is a pressure-gated phenomenon: the boundary between stability and
degradation can be controlled through multiple independent axes. This suggests
that recursive synthetic training can be governed as an engineering challenge,
provided that effective training pressure is monitored and maintained below the
relevant backbone-dependent threshold.

Several directions for future work emerge from these findings. Scaling the
