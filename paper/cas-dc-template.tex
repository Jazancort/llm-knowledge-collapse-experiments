...
$K_0$. For example, for Qwen~2.5~1.5B, $|K_0|=79$ out of 200 held-out
questions in the main rank sweep; subsequent experiments (rank $\times$ LR
matrix and interventions) obtain $|K_0|=78$ due to a minor normalization
difference affecting one borderline item. Results are always reported relative
to the $K_0$ established within each experimental batch. Other backbones have
their own fixed $K_0$ sets, determined once at generation~0 and held constant
across all experimental conditions for that backbone.
...
\caption{QLoRA rank sweep on Qwen~2.5~1.5B-Instruct. Headline ranks ($r=16$,
$r=256$) evaluated with three seeds; others with a single
