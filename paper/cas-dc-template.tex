...
\begin{figure}[htbp]
  ...
  \caption{(Top) K0 retention trajectories over 10 recursive generations for
  three representative ranks on Qwen~2.5~1.5B. Thin lines show individual seeds;
  bold lines show the mean. (Bottom) Dose-response relationship: (a)~Gen10
  retention versus nominal rank; (b)~mean effective rank versus nominal rank.
  Retention declines monotonically with increasing effective update capacity; the
  sharpest drop occurs between $r=128$ (bounded) and $r=256$
  (degradative).}\label{fig:trajectories}
\end{figure}
