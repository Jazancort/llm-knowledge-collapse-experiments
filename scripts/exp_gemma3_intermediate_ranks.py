"""Gemma 3 intermediate ranks: locate threshold between r=4 (stable) and r=16 (degradative).

Tests r=10, r=12, r=14 to find where the transition occurs.

Run: uv run python scripts/exp_gemma3_intermediate_ranks.py
"""
...
def run_single_rank(rank, train_questions, eval_questions, eval_answers, k0_indices, k0_questions, k0_answers, output_dir):
    print(f"\n{'='*60}")
    ...
    # Gen 0: generate initial synthetic
...
def main():
    output_dir = Path("outputs/gemma3_intermediate_ranks")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Load data (same as sprint2_gemma)
    ...
    all_results = {}
    for rank in RANKS:
        result = run_single_rank(rank, train_questions, eval_questions, eval_answers,
                                 k0_indices, k0_questions, k0_answers, output_dir)
        all_results[rank] = result

    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY — Gemma 3 Intermediate Ranks")
    print("=" * 60)
    print(f"  K0 = {len(k0_indices)}")
    print(f"  {'Rank':<6} {'Gen1':<8} {'Gen3':<8} {'Gen5':<8} {'erank':<8}")
    for rank in RANKS:
        gens = all_results[rank]["generations"]
        g1 = gens[0]["retention"]
        g3 = gens[2]["retention"]
        g5 = gens[-1]["retention"]
        er = gens[-1]["eff_rank"]
        k0 = all_results[rank]["k0_size"]
        print(f"  r={rank:<4} {g1}/{k0:<6} {g3}/{k0:<6} {g5}/{k0:<6} {er:.2f}")
    print(f"\n  Context: r=4 → ~93%, r=16 → ~69%")
