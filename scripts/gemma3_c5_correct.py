"""Gemma 3 C5 Intervention — CORRECT VERSION.

This is sprint2_gemma.py with ONE change: 5% random downsample of the synthetic
training data each generation. Everything else is IDENTICAL.

Same prompt, same dataset split, same K0, same eval, same generate_synthetic,
same training args, same seed handling.

Purpose: Test whether 5% exposure reduction at the Gemma 3 r=16 boundary
(which degrades to ~68.8% without intervention) restores stability.

Run on Athena:
  uv run python scripts/gemma3_c5_correct.py --rank 16 --seed 15 --generations 5
  uv run python scripts/gemma3_c5_correct.py --rank 16 --seed 137 --generations 5
  uv run python scripts/gemma3_c5_correct.py --rank 16 --seed 256 --generations 5
"""
...
def generate_synthetic(model, tokenizer, questions, seed_offset=0):
    # SAME as sprint2_gemma.py — batched, returns full decoded sequences
...
def downsample_synthetic(synthetic, seed, gen):
    """Remove ~5% of synthetic examples randomly. Only change from sprint2_gemma."""
    rng = np.random.default_rng(seed * 1000 + gen)
    n = len(synthetic)
    n_keep = int(n * DOWNSAMPLE_FRACTION)
    indices = rng.choice(n, size=n_keep, replace=False)
    indices.sort()
    return [synthetic[i] for i in indices]
...
def main():
    ...
    output_dir = Path(__file__).parent.parent / "outputs" / f"gemma3_c5correct_rank{rank}_seed{seed}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print(f"GEMMA 3 C5 CORRECT — r={rank}, seed={seed}, {NUM_GENERATIONS} gens")
    print(f"  Downsample: {DOWNSAMPLE_FRACTION*100:.0f}% kept each gen")
    print("=" * 60)

    torch.manual_seed(seed)
    np.random.seed(seed)

    # Load data — SAME as sprint2_gemma.py
    ...
    for gen in range(start_gen, NUM_GENERATIONS + 1):
        t0 = time.time()
        print(f"\n[Gen {gen}] (r={rank}, C5 downsample)")
        model, tokenizer = load_base()

        # === THE ONLY CHANGE: downsample before training ===
        train_synthetic = downsample_synthetic(prev_synthetic, seed, gen)
        print(f"    Downsample: {len(train_synthetic)}/{len(prev_synthetic)} examples kept")
        # === END OF CHANGE ===
        ...
        results.append({"generation": gen, "retention": ret,
                        ...
                        "train_examples_used": len(train_synthetic),
                        "time_sec": elapsed})
        with open(results_path, "w") as f:
    ...
    # Summary
    print("\n" + "=" * 60)
    print(f"GEMMA 3 C5 CORRECT — r={rank}, seed={seed}")
    print(f"  K0 size: {len(k0_indices)}")
    print(f"  Baseline (no intervention): seed 15={33}/47, seed 137={32}/47, seed 256={32}/47")
    for r in results[1:]:
        print(f"  Gen {r['generation']}: {r['retention']}/{len(k0_indices)} "
              f"({r['retention']/len(k0_indices):.1%})")
    print("=" * 60)
