"""FFT vs QLoRA: Causal control for the PEFT protection mechanism.

Demonstrates that full fine-tuning collapses under the same recursive
synthetic protocol where QLoRA r=16 remains homeostatic.

Run on Athena (RTX 4000 Ada 20GB):
  uv run python scripts/fft_vs_qlora.py --method fft --seed 15 --generations 5
  uv run python scripts/fft_vs_qlora.py --method qlora --seed 15 --generations 5
"""
...
def load_model(method):
    """Load model: 4-bit for QLoRA, bf16 for FFT."""
    if method == "qlora":
...
def compute_weight_drift(model, initial_state):
    """Compute L2 norm of weight change from initial state (for FFT)."""
    total_drift = 0.0
    total_norm = 0.0
    for name, param in model.named_parameters():
        if name in initial_state:
            diff = (param.data.float().cpu() - initial_state[name]).norm().item()
            total_drift += diff ** 2
            total_norm += initial_state[name].norm().item() ** 2
    return {"absolute_drift": total_drift ** 0.5, "relative_drift": (total_drift / (total_norm + 1e-10)) ** 0.5}
...
def compute_lora_spectrum(model):
    ...
    return {"eff_rank": np.mean(ranks)} if ranks else None


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--method", choices=["fft", "qlora"], required=True)
    parser.add_argument("--seed", type=int, default=15)
    parser.add_argument("--generations", type=int, default=5)
    args = parser.parse_args()

    output_dir = Path(__file__).parent.parent / "outputs" / f"fft_comparison_{args.method}_seed{args.seed}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print(f"FFT vs QLoRA: method={args.method}, seed={args.seed}, {args.generations} gens")
    print("=" * 60)
    ...
    # Gen 0 — baseline (same for both methods)
    if start_gen == 0:
        print(f"\n[Gen 0] Base model (4-bit for eval)...")
        t0 = time.time()
        model, tokenizer = load_model("qlora")  # Always eval base in 4-bit
        ...
        prev_synthetic = generate_synthetic(model, tokenizer, train_questions, seed_offset=args.seed)
        ...
        results = [{"generation": 0, "accuracy": acc, "k0_size": len(k0_indices),
                    ...
                    "transitions": None, "method_info": None}]
    ...
    for gen in range(start_gen, args.generations + 1):
        t0 = time.time()
        print(f"\n[Gen {gen}] ({args.method})")
        model, tokenizer = load_model(args.method)

        # Save initial state for FFT drift measurement
        initial_state = None
        if args.method == "fft":
            initial_state = {n: p.data.float().cpu().clone() for n, p in model.named_parameters() if p.requires_grad}
            trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        else:
            torch.manual_seed(args.seed + gen)
        ...
        # Training args — matched effective batch size, conservative LR for FFT
        lr = 2e-6 if args.method == "fft" else 1e-5
        trainer = Trainer(
            model=model,
            args=TrainingArguments(
                output_dir=str(output_dir / "tmp"), num_train_epochs=2,
                per_device_train_batch_size=2, gradient_accumulation_steps=8,
                learning_rate=lr, bf16=True, logging_steps=100,
                save_strategy="no", report_to="none", seed=args.seed + gen,
                gradient_checkpointing=True,
                optim="paged_adamw_8bit" if args.method == "fft" else "adamw_torch",
            ),
        ...
        if args.method == "fft":
            drift = compute_weight_drift(model, initial_state)
            method_info = {"weight_drift": drift, "trainable_params": trainable}
            print(f"    Weight drift: abs={drift['absolute_drift']:.4f} rel={drift['relative_drift']:.6f}")
            del initial_state
        else:
            spectrum = compute_lora_spectrum(model)
            method_info = {"eff_rank": spectrum["eff_rank"], "trainable_params": trainable}
            print(f"    Effective rank: {spectrum['eff_rank']:.2f} / 16")
        ...
        print("    Generating synthetic...")
        new_synthetic = generate_synthetic(model, tokenizer, train_questions, seed_offset=args.seed + gen + 100)
        ...
        results.append({"generation": gen, "retention": ret,
                        "transitions": trans, "method_info": method_info,
                        "k0_results": k0_res, "time_sec": elapsed})
        with open(results_path, "w") as f:
    ...
    # Summary
    print("\n" + "=" * 60)
    print(f"FFT vs QLoRA FINAL ({args.method})")
    ...
    for r in results:
        ...
        if mi:
            if "eff_rank" in mi:
                extra = f"eff_rank={mi['eff_rank']:.1f}"
            elif "weight_drift" in mi:
                extra = f"drift={mi['weight_drift']['relative_drift']:.6f}"
        t = r.get("transitions")
        cw = t["C->W"] if t else "-"
        wc = t["W->C"] if t else "-"
        print(f"  Gen {gen}: {ret}/{k0} ({ret/k0*100:.1f}%) {extra} C->W={cw} W->C={wc}")
